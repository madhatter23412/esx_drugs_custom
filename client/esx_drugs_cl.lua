...
local moonshineQTE					= 0
local moonshine_poochQTE 			= 0
local myJob 					= nil
local PlayerData 				= {}
local GUI 						= {}
...
AddEventHandler('esx_drugs:hasEnteredMarker', function(zone)
		...
		--Moonshine
        if zone == 'MoonshineFarm' then
            if myJob ~= "police" then
                CurrentAction     = 'moonshine_harvest'
                CurrentActionMsg  = _U('press_collect_moonshine')
                CurrentActionData = {}
            end
        end

        if zone == 'MoonshineTreatment' then
            if myJob ~= "police" then
                if moonshineQTE >= 5 then
                    CurrentAction     = 'moonshine_treatment'
                    CurrentActionMsg  = _U('press_process_moonshine')
                    CurrentActionData = {}
                end
            end
        end

        if zone == 'MoonshineResell' then
            if myJob ~= "police" then
                if moonshine_poochQTE >= 1 then
                    CurrentAction     = 'moonshine_resell'
                    CurrentActionMsg  = _U('press_sell_moonshine')
                    CurrentActionData = {}
                end
            end
        end
...
AddEventHandler('esx_drugs:hasExitedMarker', function(zone)
		...
		TriggerServerEvent('esx_drugs:stopHarvestMoonshine')
        TriggerServerEvent('esx_drugs:stopTransformMoonshine')
        TriggerServerEvent('esx_drugs:stopSellMoonshine')
end)
...
AddEventHandler('esx_drugs:ReturnInventory', function(cokeNbr, cokepNbr, methNbr, methpNbr, weedNbr, weedpNbr, opiumNbr, opiumpNbr, moonshineNbr, moonshinepNbr, jobName, currentZone)
    cokeQTE       = cokeNbr
	...
	moonshineQTE       = moonshineNbr
    moonshine_poochQTE = moonshinepNbr
    myJob         = jobName
        ...
        if CurrentAction ~= nil then
            ...
            if IsControlJustReleased(0, 38) then
                ...
                if CurrentAction == 'opium_resell' then
				 ...
				 if CurrentAction == 'moonshine_harvest' then
                    TriggerServerEvent('esx_drugs:startHarvestMoonshine')
                end
                if CurrentAction == 'moonshine_treatment' then
                    TriggerServerEvent('esx_drugs:startTransformMoonshine')
                end
                if CurrentAction == 'moonshine_resell' then
                    TriggerServerEvent('esx_drugs:startSellMoonshine')
                end
                CurrentAction = nil
            end
        end
