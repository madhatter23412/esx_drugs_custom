...
local function HarvestMoonshine(source)
	...
	if CopsConnected < Config.RequiredCopsMoonshine then
		TriggerClientEvent('esx:showNotification', source, _U('act_imp_police', CopsConnected, Config.RequiredCopsMoonshine))
		return
	end
	...
	SetTimeout(5000, function()
		...
		if PlayersHarvestingMoonshine[source] == true then
			...
			if moonshine.limit ~= -1 and moonshine.count >= moonshine.limit then
				TriggerClientEvent('esx:showNotification', source, _U('inv_full_moonshine'))
			else
...
local function TransformMoonshine(source)
	...
	SetTimeout(10000, function()
		...
		if PlayersTransformingMoonshine[source] == true then
			...
			if poochQuantity > 35 then
				TriggerClientEvent('esx:showNotification', source, _U('too_many_pouches'))
			elseif moonshineQuantity < 3 then
				TriggerClientEvent('esx:showNotification', source, _U('not_enough_moonshine'))
			else
				xPlayer.removeInventoryItem('moonshine', 5)
	...
	SetTimeout(7500, function()
		...
		if PlayersSellingMoonshine[source] == true then
			...
			if poochQuantity == 0 then
				...
				if CopsConnected == 0 then
                    xPlayer.addAccountMoney('black_money', 300)
                    TriggerClientEvent('esx:showNotification', source, _U('sold_one_moonshine'))
                elseif CopsConnected == 1 then
